...
// Batched from day one. Use ?start=0&end=5, then 5-10, 10-15, 15-20.
export default async function handler(req, res) {
  const secret = req.query.secret || req.headers['authorization']?.replace('Bearer ','')
  if (secret !== (process.env.CRON_SECRET || 'findvibe_secret_2026')) return res.status(401).json({error:'Unauthorized'})
