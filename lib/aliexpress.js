...
export const CATEGORY_LIST = [
  ...
  { id: 'textiles',       keywords: ['boho cushion cover aesthetic','nordic area rug minimalist','throw pillow cover'] },
  ...
  { id: 'mirrors',        keywords: ['irregular shape mirror aesthetic','vanity led mirror minimalist','wall mirror decorative'] },
  { id: 'plants-nature',  keywords: ['artificial plant decor aesthetic','ceramic flower vase minimalist','flower vase decoration'] },
  { id: 'aroma',          keywords: ['ultrasonic aroma diffuser aesthetic','scented candle minimalist'] },
  { id: 'furniture',      keywords: ['velvet ottoman aesthetic','minimalist side table nordic','small side table'] },
  { id: 'clocks',         keywords: ['minimalist wall clock aesthetic','silent wall clock decor'] },
  { id: 'entryway',       keywords: ['aesthetic key holder minimalist','shoe storage bench nordic','entryway organizer rack'] },
  { id: 'home-office',    keywords: ['desk mat aesthetic minimalist','ergonomic monitor stand aesthetic','desk accessories organizer'] },
  ...
  { id: 'atmosphere',     keywords: ['led neon sign room aesthetic','string lights minimalist','led strip light room'] },
  ...
  { id: 'luxury-accents', keywords: ['gold finish decor aesthetic','marble style coaster minimalist','gold decor accent piece'] },
]
...
const EXCLUDE_WORDS = ['cheap','plastic','toy','wholesale','bulk','10pcs','set of','wristwatch','wrist watch','smartwatch','smart watch']
...
// Vocabulary for building a short, elegant display title instead of showing
// AliExpress's long literal product title. We extract the material + object
// type the title actually contains, then assemble "[Material] [Object]" -
// e.g. "Stainless Steel Double S-Shaped Storage Hook for Bathroom..." -> "Minimalist Stainless Hook".
const MATERIALS = ['stainless steel','ceramic','wood','wooden','marble','velvet','silk','acrylic','glass','rattan','linen','bamboo','brass','copper','resin','cotton']
const OBJECTS = [
  'hook','lamp','vase','mirror','organizer','clock','candle','diffuser','shelf','tray',
  'curtain','rug','cushion','pillow','mug','plate','bowl','holder','stand','rack',
  'storage','box','basket','planter','sconce','light','ottoman','stool','table',
  'frame','bag','bin','dispenser','bench','panel','art','sign','garland',
]
const ADJECTIVES = ['Minimalist','Modern','Nordic','Aesthetic','Sculptural','Refined']

function viralTitle(rawTitle) {
  const lower = rawTitle.toLowerCase()
  const material = MATERIALS.find(m => lower.includes(m))
  const object = OBJECTS.find(o => lower.includes(o))
  const adj = ADJECTIVES[Math.floor(Math.random() * ADJECTIVES.length)]

  if (material && object) {
    const matLabel = material.charAt(0).toUpperCase() + material.slice(1)
    const objLabel = object.charAt(0).toUpperCase() + object.slice(1)
    return `${matLabel} ${objLabel}`
  }
  if (object) {
    const objLabel = object.charAt(0).toUpperCase() + object.slice(1)
    return `${adj} ${objLabel}`
  }
  // Fallback: first 3 clean words, title-cased
  const words = cleanTitle(rawTitle).split(/\s+/).slice(0, 3)
...
export function filterProducts(rawProducts, existingTitles = []) {
  ...
  for (const p of sorted) {
    ...
    if (vol < 200) continue // popularity floor - relaxed from 1000 since niche aesthetic keywords have lower volume
...
export async function queryAPI(keywords, cat, page=1, existingTitles=[]) {
  ...
  return clean.map(p => ({
    ...
    title: viralTitle(p.product_title),
